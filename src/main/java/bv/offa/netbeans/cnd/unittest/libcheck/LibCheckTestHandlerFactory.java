...
/**
 * The class {@code LibCheckTestHandlerFactory} implements a factory for
 * <i>LibCheck</i> test handler.
 *
 * @author offa
 */
public class LibCheckTestHandlerFactory implements TestHandlerFactory
{
    ...
    {
        Manager.getInstance().setTestingFramework(TestFramework.LibCheck.getName());
        Manager.getInstance().setNodeFactory(new TestRunnerUINodeFactory());

        List<TestRecognizerHandler> testHandler = new ArrayList<>();
        testHandler.add(new LibCheckTestFinishedHandler());
        testHandler.add(new LibCheckTestSessionFinishedHandler());
        return testHandler;
    }
